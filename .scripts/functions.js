...
folders.forEach(folder => {
	...
	// loop on each files
	files.forEach(file => {
		const name = "Thorin::" + path.basename(file).replace(".php", "");
	...
	files.forEach(file => {
		...
		docblocks.forEach(docblock => {
			const name = "Thorin::" + docblock.name;
			...
			mds.push("");
			mds.push(
				`[Full documentation](/doc/${file.replace(".php", ".md")})`
			);
			if (docblock.see) {
				mds.push("");
	...
	// create file
	fs.writeFileSync(
		`doc/functions/${path.basename(folder)}.md`,
		mds.join("\n")
	);
});
