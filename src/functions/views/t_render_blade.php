...
function t_render_blade($slug, $data = []) {
	...
	if (is_dir(t_sanitize_path(Thorin::config('views.VIEWS_PATH')) . $slug)) {
		$parts = explode('/', $slug);
		$slug = $slug . '/' . $parts[count($parts)-1];
	}
	// generate blade
    $blade = new Blade( t_sanitize_path(Thorin::config('views.VIEWS_PATH')), t_sanitize_path(Thorin::config('views.CACHE_PATH')));
