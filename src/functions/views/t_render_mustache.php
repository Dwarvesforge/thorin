 ...
 */
function t_render_mustache($slug, $data = []) {
	...
	if ( ! $t_mustache_engine) {
		$t_mustache_engine = new Mustache_Engine(array(
			...
			'loader' => new Mustache_Loader_FilesystemLoader(Thorin::sanitize_path(Thorin::config('paths.views'))),
			'partials_loader' => new Mustache_Loader_FilesystemLoader(Thorin::sanitize_path(Thorin::config('paths.views'))),
			'helpers' => [],
